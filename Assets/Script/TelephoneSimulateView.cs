...
public class TelephoneSimulateView : MonoBehaviour
{
    ...
    public List<UIDragHandler> mNumDragList;
    ...
    void Awake()
    {
        ...
        foreach (UIDragHandler drag in this.mNumDragList)
        {
            int num =int.Parse(drag.name);
            this.mNumDragDic[num] = drag.GetComponent<RectTransform>();
            drag._downCall = this.OnPointerDown;
            drag._beginCall = this.OnBeginDrag;
            drag._dragCall = this.OnDrag;
            drag._endCall = this.OnEndDrag;
            drag.SetParam(num);
            Vector2 orignPos = new Vector2(this.mNumDragDic[num].anchoredPosition.x, this.mNumDragDic[num].anchoredPosition.y);
            this.mNumDragAnchorPosDic[num] = orignPos;
        }
    ...
    public void ResetDrager(bool enable = true)
    {
        ...
        foreach (UIDragHandler drag in this.mNumDragList)
        {
            ...
            int num = int.Parse(drag.name);
            mNumDragDic[num].anchoredPosition = new Vector2(this.mNumDragAnchorPosDic[num].x, this.mNumDragAnchorPosDic[num].y);
        }
    }
    ...
    {
        foreach (int key in this.mNumDragDic.Keys)
        {
            this.mNumDragDic[key].GetComponent<UIDragHandler>().SetEnable(key == index);
        }
    }
